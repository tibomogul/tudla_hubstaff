...
module TudlaHubstaff
  RSpec.describe FetchUpdatedActivitiesService do
    let(:api_client) { instance_double(ApiClient) }
    let(:start_time) { "2025-12-22T00:00:00Z" }
    let(:stop_time) { "2025-12-22T23:59:59Z" }
    ...
    describe "#call" do
      context "when API returns activities without pagination" do
        ...
        before do
          allow(api_client).to receive(:daily_activities_by_updated)
      ...
      context "when API returns paginated results" do
        ...
        it "fetches all pages until no next_page_start_id" do
          expect(api_client).to receive(:daily_activities_by_updated)
